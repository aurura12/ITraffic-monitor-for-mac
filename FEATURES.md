...
- 按进程实时显示上传 / 下载速度
- 原生 macOS 菜单栏常驻入口：点击图标查看实时下载 / 上传速度，可打开完整仪表盘
- 原生 macOS 仪表盘界面，浅色 / 深色模式自适应
- 直接驱动系统 `nettop`，无需额外打包二进制
- Delta 模式采样，实时速率更准确
- 本地 SQLite 历史流量记录，附可视化仪表盘
- 数据支持 CSV / JSON 导出
- VPN / 代理流量按 App 归属为尽力而为的估算；默认免费构建不包含需要签名授权的 Network Extension 精确统计
...
流量按“分钟分桶”写入本地 SQLite（WAL 模式），永久保留，不按天数自动清理。仪表盘窗口共 6 个标签页：
